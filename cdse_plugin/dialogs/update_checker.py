"""Update checker dialog for CDSE Plugin.
...
class VersionCheckWorker(QThread):
    ...
    Signals:
        finished: Emitted when check completes with result dict.
...
class DownloadWorker(QThread):
    ...
    Signals:
        finished: Emitted when installation completes with install path.
        error: Emitted when an error occurs with error message.
        progress: Emitted with progress percent and status message.
    """
    ...
    def __init__(self, plugin_dir: str, parent=None):
        ...
        Args:
            plugin_dir: Path to the current plugin directory.
    ...
    def run(self) -> None:
        ...
        try:
            ...
            temp_dir = tempfile.mkdtemp(prefix="cdse_plugin_update_")
            ...
            # The structure will be: repo-main/cdse_plugin/
            extracted_plugin_dir = None
            for root, dirs, files in os.walk(extract_dir):
                if "metadata.txt" in files and "cdse_plugin.py" in files:
            ...
            try:
                ...
                def is_valid_plugin_dir(path):
                    ...
                    ) and os.path.exists(os.path.join(path, "cdse_plugin.py"))
...
class UpdateCheckerDialog(QDialog):
    ...
    def __init__(self, plugin_dir: str, parent=None):
        """Initialize the update checker dialog.
        ...
        self.setWindowTitle("CDSE Plugin Update")
    ...
    def _setup_ui(self) -> None:
        ...
        # Header
        header_label = QLabel("CDSE Plugin Update Checker")
        header_font = QFont()
    ...
    def _on_check_finished(self, result: dict) -> None:
        ...
        Args:
            result: Dict with version, changelog, and metadata.
        """
        self.progress_bar.setVisible(False)
    ...
    def download_and_install(self) -> None:
        ...
        self.download_worker = DownloadWorker(self.plugin_dir, self)
    ...
    def _on_download_finished(self, install_path: str) -> None:
        ...
        QMessageBox.information(
            self,
            "Update Complete",
            f"CDSE Plugin has been updated to version {self.latest_version}.\n\n"
...
def get_current_version() -> str:
    """Get the current plugin version from metadata.

    Returns:
        Version string or None if not found.
    """
    metadata_path = os.path.join(get_plugin_dir(), "metadata.txt")
...
def check_for_updates(parent=None, silent: bool = False) -> None:
    """Start an update check (silent mode for startup).
    ...
    """
    if silent:
        # Do a quick background check and only show dialog if update available
        _silent_check(parent)
    else:
        # Show the full update checker dialog
        show_update_dialog(parent)


def _silent_check(parent=None) -> None:
    """Perform a silent update check.
    ...
    # Keep worker reference to prevent garbage collection
    if parent:
